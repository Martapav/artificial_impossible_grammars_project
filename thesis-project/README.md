...
```
thesis-project/
├── lexicon/              Lexicon package + CLI
├── lexicon.json          Shared generated lexicon
├── grammars/
│   ├── base_generator.py Grammar-agnostic base class
│   ├── grammar_H/        Hierarchical
│   ├── grammar_P/        Positional
│   ├── grammar_H_prime/  H + positional wh rule
│   └── grammar_L_prime/  P + structural wh rule
├── corpora/{train,test_indistribution,test_generalization}/
├── models/{configs,checkpoints}/     Phase 1 (not started)
├── das/{interventions,results}/      Phase 2 (not started)
├── evaluation/{phase1,phase2}/       (not started)
├── tests/                Pytest suite
├── scripts/              build_all_corpora.py
...
| Component | Status |
|---|---|
| Lexicon | Done — reproducible from seed 42 |
| Grammars H, P, H′, L′ | Done — base + 3 transformations + generalization sets |
| Corpus builder | Done — all four grammars wired through `build_all_corpora.py` |
| Model training / DAS / evaluation | Not started |
| Tests | Lexicon + grammar generation/determinism passing (2 pre-existing lexicon-serialization failures, unrelated) |

**Known limitations:** uniform lexical sampling (Zipfian frequency not yet
applied); the in-distribution test split is produced by re-seeding (`seed+100000`)
rather than reserving disjoint base sentences; at most one transformation per
sentence (no phenomenon co-occurrence yet).
